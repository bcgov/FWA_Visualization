...
$( document ).ready(function() {
  ...
  function addStreamLabelOnStream(e) {
    var layer = e.target;
      var river = layer.feature;
      //var riverId = river.properties.id;
      if (river) {
        var highlightedRiverMarker = L.marker(layer.getBounds().getCenter(), {
          clickable: false,
          icon: L.divIcon({
            className: 'leaflet-div-icon',
            html: "<b>" + String(river.properties.name) + "</b>" + "<br>Segment length: " + (river.properties.seglen/1000).toFixed(1) + " km" + "<br>Upstream length: " + (river.properties.upslen/1000).toFixed(1) + " km" + "<br>Downstream length: " + (river.properties.dwnslen/1000).toFixed(1) + " km",
            iconSize: [200, 100]
            //popupAnchor: [200, 200] //TODO: figure this out
          })
        });
        markers.addLayer(highlightedRiverMarker);
        map.addLayer(markers);
        
        //remove label from map on 'mouseout'
        layer.on("mouseout", function(e) {
          markers.removeLayer(highlightedRiverMarker);
          map.removeLayer(markers);
        });
      }
    }
    
  function addStreamMarkerOnStream(e) {
    var layer = e.target;
      var river = layer.feature;
      //var riverId = river.properties.id;
      if (river) {
        ...
        markers.addLayer(highlightedRiverMarker);
        map.addLayer(markers);
        highlightedRiverMarker.bindPopup(popup).openPopup();
  ...
  function addStreamLabelInMap(e) {
    var layer = e.target;
      var river = layer.feature;
      //var riverId = river.properties.id;
      if (river) {
        //var centerCoords = map.getBounds().getCenter();
        //console.log(centerCoords);
        var centerLatCoord = map.getBounds().getCenter().lat;
        //console.log(centerLatCoord);
        var centerLongCoord = map.getBounds().getCenter().lng;
        //console.log(centerLongCoord);
        var longCoord = map.getBounds().getWest() - (map.getBounds().getWest())*.015;
        //var longCoord = map.getBounds().getWest() + 3.0;
        //console.log(longCoord);
        var latCoord = map.getBounds().getSouth() + (map.getBounds().getSouth())*.0015;
        //console.log(latCoord);
        var streamLabelPosition = {lat: centerLatCoord, lng: longCoord};
        //var streamLabelPosition = {lat: latCoord, lng: centerLongCoord};
        //console.log(streamLabelPosition);
        var highlightedRiverMarker = L.marker(streamLabelPosition, {
          clickable: false,
          icon: L.divIcon({
            className: 'leaflet-div-icon',
            html: "<b>" + String(river.properties.name) + "</b>" + "<br>Segment length: " + (river.properties.seglen/1000).toFixed(1) + " km" + "<br>Upstream length: " + (river.properties.upslen/1000).toFixed(1) + " km" + "<br>Downstream length: " + (river.properties.dwnslen/1000).toFixed(1) + " km",
            ...
            iconSize: [200, 100]
            //popupAnchor: [-300, 0] //TODO: figure this out
          })
        });
        markers.addLayer(highlightedRiverMarker);
        map.addLayer(markers);
